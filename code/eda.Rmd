...
not. Analysis of these correlations can help in finding the rankings of the attributes.
The final answer to the research question will be found using Weka, a data
...
be observed in a later section. Any pre-processing or cleanup required will
...
ggplot(main, aes(x=factor(stroke), y=age))+geom_boxplot()+ggtitle("Ages by stroke history")+
  xlab("Stroke")
```
\newpage
...
text variation. Changing the nearest neighbour search algorithm from the default
...
the accuracy to a significnat degree is the number of iteratoins, which gives 
